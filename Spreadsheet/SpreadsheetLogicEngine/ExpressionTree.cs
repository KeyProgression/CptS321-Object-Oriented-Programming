namespace CptS321
{
    ...
    /// <param name="expression">A string.</param>
        ...
        /// <summary>
        /// Gets the variable for expressions
        /// </summary>
        /// <param name="expression"></param>
        /// <returns>Returns all the variables in collection.</returns>
        public List<string> GetVar(string expression)
        {
            ...
            for (int index = 0; index < expression.Length; index++)
            {
                if (char.IsLetter(expression[index]))
                {
                    ...
                    {
                        if (char.IsDigit(temp[jndex]))
                        {
                            name += temp[jndex];
                        }

                        if (char.IsLetter(temp[jndex] || temp[jndex] == '+' || temp[jndex] == '-' || temp[jndex] == '/'))
                        {
                            // do notaing
                            break;
                        }
